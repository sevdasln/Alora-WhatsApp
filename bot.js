...
async function whatsAsena () {
    ...
    var insult = await axios.get('https://gist.githubusercontent.com/xYAZUWA/b8a58ad263e5a82d5764dec53f68a3a1/raw')
    const { shs1, shl2, lss3, dsl4 } = insult.data.inside
    await config.DATABASE.sync();
    var StrSes_Db = await AloraDB.findAll({
        where: {
          info: 'StringSession'
        }
    });
    ...
    conn.on ('open', async () => {
        ...
        if (StrSes_Db.length < 1) {
            await AloraDB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*Alora Working as Public! 🐺*\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*Your bot working as public. To change it, use* _.setvar WORK_TYPE:private_\n\n*Thanks for using Alora 💌*', MessageType.text);
                }               
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') { 
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*Alora Working as Private! 💎*\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*Your bot working as private. To change it, use* _.setvar WORK_TYPE:public_\n\n*Thanks for using Alora 💌*', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                    ...
                    await conn.sendMessage(
                        conn.user.jid,
                        '```Type``` *.update now* ```For The Update Bot.```\n\n' + degisiklikler + '```\n*Dont Update Until Support Group Notices You*', MessageType.text
                    ); 
                }
            }
        }
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [αℓσяα] --*' + 
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [αℓσяα] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
